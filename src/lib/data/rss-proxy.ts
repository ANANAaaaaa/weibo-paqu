...
// 微博用户ID到用户名的映射
  ...
  async fetchAllFeeds(forceRefresh: boolean = false): Promise<RSSItem[]> {
    ...
    // 串行处理避免被限流，但限制处理数量以避免超时
    const maxUsers = Math.min(userIds.length, 5) // 限制最多处理5个用户
    ...
    if (allItems.length === 0) {
      console.log('RSS获取失败，返回模拟数据')
      return this.getMockData()
    }

    console.log(`RSS获取完成: ${allItems.length} 条数据，成功 ${successCount}/${maxUsers} 个源`)

    // 按时间倒序排序
    return allItems.sort((a, b) => new Date(b.pubDate).getTime() - new Date(a.pubDate).getTime())
  }

  private getMockData(): RSSItem[] {
    const now = Date.now()
    return [
      {
        id: `mock_1_${now}`,
        title: '今日热点话题：AI技术发展趋势分析',
        summary: '深度解析当前AI技术的发展现状和未来趋势，包括大模型、自动驾驶等热门领域。',
        description: '深度解析当前AI技术的发展现状和未来趋势，包括大模型、自动驾驶等热门领域。',
        link: 'https://weibo.com/mock/1',
      ...
      {
        id: `mock_2_${now}`,
        title: '娱乐圈最新动态：热门影视作品盘点',
        summary: '盘点近期热播的影视作品，分析其成功因素和市场表现。',
        description: '盘点近期热播的影视作品，分析其成功因素和市场表现。',
        link: 'https://weibo.com/mock/2',
      ...
      {
        id: `mock_3_${now}`,
        title: '生活方式分享：健康饮食新理念',
        summary: '分享健康饮食的新理念和实用技巧，帮助大家建立更好的生活习惯。',
        description: '分享健康饮食的新理念和实用技巧，帮助大家建立更好的生活习惯。',
        link: 'https://weibo.com/mock/3',
      ...
      },
      {
        id: `mock_4_${now}`,
        title: '财经观察：市场趋势解读',
        summary: '解读当前市场的主要趋势和投资机会，为投资者提供参考。',
        description: '解读当前市场的主要趋势和投资机会，为投资者提供参考。',
        link: 'https://weibo.com/mock/4',
      ...
      },
      {
        id: `mock_5_${now}`,
        title: '教育话题：在线学习的新模式',
        summary: '探讨在线教育的发展趋势和新的学习模式，分析其优势和挑战。',
        description: '探讨在线教育的发展趋势和新的学习模式，分析其优势和挑战。',
        link: 'https://weibo.com/mock/5',
